...
def main(argv=None):
    ...
    parser = argparse.ArgumentParser(
        description='For Nuxeo collection, stash thumbnails in S3.')
    ...
    for obj in objects:
        nxstash = NuxeoStashThumb(obj['path'], argv.bucket, argv.region,
                                  argv.pynuxrc, argv.replace)
        report[nxstash.uid] = nxstash.nxstashref()
        for c in dh.fetch_components(obj):
            nxstash = NuxeoStashThumb(c['path'], argv.bucket, argv.region,
                                      argv.pynuxrc, argv.replace)
            report[nxstash.uid] = nxstash.nxstashref()
    ...
    not_file = len([
        ...
        if not value['calisphere_type'] == 'file'
    ])
