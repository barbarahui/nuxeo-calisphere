...
class DeepHarvestNuxeoSimpleTestCase(unittest.TestCase):
    def setUp(self):
        ...
        self.dh = deepharvest_nuxeo.DeepHarvestNuxeo(
            self.path, self.s3_mediajson, pynuxrc=self.pynuxrc)

        test_json_dir = os.path.join(
            os.path.abspath(os.path.dirname(__file__)), 'json')
        self.test_simple_object = os.path.join(test_json_dir,
                                               'simple_object.json')
        self.test_complex_object_depth_one = os.path.join(
            test_json_dir, 'complex_object_one_deep.json')
        self.test_complex_object_depth_two = os.path.join(
            test_json_dir, 'complex_object_two_deep.json')
        self.test_component_object = os.path.join(test_json_dir,
                                                  'component_object.json')
        self.test_component_with_transcript = os.path.join(
            test_json_dir, 'component_object_with_transcript.json')
        self.test_nuxeo_full_md_transcript = os.path.join(
            test_json_dir, 'nuxeo_full_metadata_transcript.json')
        self.test_nuxeo_full_metadata = os.path.join(
            test_json_dir, 'nuxeo_full_metadata.json')
        self.test_nuxeo_organizational_object = os.path.join(
            test_json_dir, 'nuxeo_organizational_object.json')
        self.test_nuxeo_empty_folder_object = os.path.join(
            test_json_dir, 'nuxeo_empty_folder_object.json')
        self.test_nuxeo_object_no_file = os.path.join(
            test_json_dir, 'nuxeo_object_no_file.json')

    def test_init(self):
        ''' test initialization of DeepHarvestNuxeo object '''
        dh = deepharvest_nuxeo.DeepHarvestNuxeo(
            self.path, self.s3_mediajson, pynuxrc=self.pynuxrc)
    ...
    def test_fetch_objects(self):
        ...
        objects = self.dh.fetch_objects(
        )  # FIXME mock this generator so we don't actually go over the network
        self.assertEqual(sum(1 for ob in objects),
                         215)  # FIXME bad test! # of objects might change...

    def test_fetch_objects_nested(self):
        ''' test fetching of Nuxeo objects for a collection that has
        nested folders '''
        # FIXME bad test! # of objects might change...
        #dh = deepharvest_nuxeo.DeepHarvestNuxeo('/asset-library/UCR/'
        # 'SabinoOsuna', self.s3_mediajson, pynuxrc=self.pynuxrc)
        #objects = dh.fetch_objects()
        #self.assertEqual(sum(1 for ob in objects), 427)
    ...
    def test_fetch_components_depth_one(self):
        ...
        components = self.dh.fetch_components(
            obj)  # mock so we don't actually go over network
    ...
    def test_fetch_components_depth_several(self):
        ...
        # mock so we don't actually go over network
        #components = self.dh.fetch_components(obj)
